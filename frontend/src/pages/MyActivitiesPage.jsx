...
export default function MyActivitiesPage() {
  ...
  return (
    <div>
      <div className="mb-3">
        <h4 className="fw-bold mb-1">Mis actividades</h4>
        <div className="text-muted small">
          Aquí ves únicamente los registros que has creado tú: visitas, llamadas, emails y actividad rápida.
        </div>
      </div>
          ...
          <div className="table-responsive">
            <table className="table table-hover table-sm align-middle">
              <thead className="table-light">
                <tr>
                  <th>Fecha</th><th>Cliente</th><th>Tipo</th><th>Producto</th><th>Resultado</th><th>Duración</th><th>Estado</th><th></th>
                </tr>
              </thead>
              <tbody>
                {activities.length === 0 && (
                  <tr><td colSpan={8} className="text-center text-muted py-3">No hay actividades</td></tr>
                )}
                {activities.map(a => {
                  ...
                  return (
                    <tr key={a._id}>
                      ...
                      <td>{a.productIds?.length ? a.productIds.map((product) => product?.name).filter(Boolean).join(', ') : (a.productId?.name || '-')}</td>
                      <td>{a.outcomeId?.name || '-'}</td>
                      <td>{a.durationMinutes ? `${a.durationMinutes} min` : '-'}</td>
                      <td><span className={`badge ${cls}`}>{label}</span></td>
                      <td className="text-end">
                        <Link to={`/activities/${a._id}`} className="btn btn-sm btn-outline-primary">
                          Ver detalle
                        </Link>
                      </td>
                    </tr>
