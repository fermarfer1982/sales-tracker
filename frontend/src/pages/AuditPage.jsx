  ...
  async function handleExport() {
    ...
    try {
      ...
      const rows = (res.data.data || []).map((log) => ({
        fecha: formatDateTime(log.at),
        entidad: log.entityName || '',
        entity_id: log.entityId || '',
        accion: log.action || '',
        usuario: log.userId?.name || '',
        email_usuario: log.userId?.email || '',
      }));
      downloadCsv(`auditoria_${new Date().toISOString().slice(0, 10)}.csv`, rows);
    } catch {}
    setLoading(false);
  }
  ...
  return (
    <div>
      <div className="d-flex justify-content-between align-items-center mb-3">
        <h4 className="fw-bold mb-0">Auditoría</h4>
        <button className="btn btn-outline-secondary btn-sm" onClick={handleExport} disabled={loading || logs.length === 0}>
          Exportar CSV
        </button>
      </div>
