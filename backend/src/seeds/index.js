...
async function seed() {
  ...
  const activityTypes = await ActivityType.insertMany([
    { name: 'visita' }, { name: 'llamada' }, { name: 'email' }, { name: 'viaje' }, { name: 'administracion' }, { name: 'feria' }, { name: 'ensayo' }, { name: 'comida' },
  ]);
  const outcomes = await Outcome.insertMany([
    { name: 'oferta' }, { name: 'pedido' }, { name: 'seguimiento' }, { name: 'incidencia' }, { name: 'impago' },
  ]);
