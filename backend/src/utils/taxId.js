...
function looksLikeSpanishTaxId(taxId) {
  if (!taxId || typeof taxId !== 'string') return false;
  const normalized = taxId.replace(/[\s\-]/g, '').toUpperCase();
  return /^(\d{8}[A-Z]|[XYZ]\d{7}[A-Z]|[ABCDEFGHJKLMNPQRSUVW]\d{7}[0-9A-J])$/.test(normalized);
}

function validateForeignTaxId(taxId) {
  if (!taxId || typeof taxId !== 'string') return false;
  const normalized = taxId.replace(/[\s\-]/g, '').toUpperCase();
...
function validateClientTaxId(taxId) {
  if (!taxId || typeof taxId !== 'string') return false;
  const normalized = taxId.replace(/[\s\-]/g, '').toUpperCase();
  if (looksLikeSpanishTaxId(normalized)) {
    return validateSpanishTaxId(normalized);
  }
  return validateForeignTaxId(normalized);
}
