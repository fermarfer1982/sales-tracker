...
async function getUserIds(req) {
  ...
  if (req.user.role === 'manager') {
    return getAccessibleSalesUserIds(req.user, { isActive: true });
  }
  const all = await User.find({ isActive: true, role: 'sales' }).select('_id');
...
async function getKpis(req, res) {
  try {
    ...
    if (scope === 'my' || req.user.role === 'sales') {
      ...
      userIds = await getAccessibleSalesUserIds(req.user, { isActive: true });
    } else {
      const all = await User.find({ isActive: true, role: 'sales' }).select('_id');
