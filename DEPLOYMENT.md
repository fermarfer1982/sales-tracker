...
- El servidor debe tener IP fija en la red local (ej: `IP_DEL_SERVIDOR`)
...
> Sustituye `usuario` por el nombre de usuario del servidor y `IP_DEL_SERVIDOR` por la IP real.
...
> `mongodb://USUARIO:PASSWORD@localhost:27017/sales_tracker`
...
```powershell
scp -r "C:\ruta\a\tu\proyecto\*" usuario@IP_DEL_SERVIDOR:/var/www/sales-tracker/
```
...
```env
PORT=3001
MONGO_URI=mongodb://USUARIO:PASSWORD@localhost:27017/sales_tracker
...
SMTP_PASS=CAMBIA_ESTA_PASSWORD_SMTP
SMTP_SECURE=false
...
server {
    listen 80;
    server_name IP_DEL_SERVIDOR;  # Cambia por tu IP o dominio local
...
Desde tu PC, abre el navegador y ve a: `http://IP_DEL_SERVIDOR`
...
sudo openssl req -x509 -nodes -days 3650 -newkey rsa:2048 \
    ...
    -subj "/C=ES/ST=Madrid/L=Madrid/O=MiEmpresa/CN=IP_DEL_SERVIDOR"
```

> Cambia `IP_DEL_SERVIDOR` por tu IP real.
...
# Nginx responde
curl -k https://IP_DEL_SERVIDOR/api/health
```
...
1. Abre `https://IP_DEL_SERVIDOR` en Chrome o Firefox
...
```bash
mongodump --uri="mongodb://USUARIO:PASSWORD@localhost:27017/sales_tracker" --out=/backup/sales-tracker-$(date +%Y%m%d)
```
...
Añade al final:
```
0 2 * * * mongodump --uri="mongodb://USUARIO:PASSWORD@localhost:27017/sales_tracker" --out=/backup/sales-tracker-$(date +\%Y\%m\%d) && find /backup -name "sales-tracker-*" -mtime +30 -exec rm -rf {} \;
```
